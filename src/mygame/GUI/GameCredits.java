...
// A custom JPanel designed to display game credits with a sci-fi theme,
// featuring animated developer cards that slide up upon display.
public class GameCredits extends JPanel {
    ...
    // Constructor: Initializes the panel, loads resources, sets up developer data,
    // creates the UI components (title, cards, button), and prepares the animation.
    ...
    // Custom painting of the background, including the space gradient and stars.
    @Override
    ...
    // Loads the font configuration.
    private void loadFont() {
        gameFont = new Font("Verdana", Font.BOLD, 18);
    }

    // Generates random positions for background stars.
    ...
    // Starts the animation sequence where cards appear one by one.
    ...
    // Animates a single developer card, making it fade in and slide up to its final position.
    ...
    // Represents a single developer's profile card, handling its content and visual appearance/animation effects.
    class DevCard extends JPanel {
        ...
        // Sets the current opacity level of the card.
        void setOpacity(float f) {
            opacity = f;
        }

        // Gets the current opacity level of the card.
        float getOpacity() {
            return opacity;
        }

        // Custom painting for the card, applying transparency and drawing the sci-fi border.
        @Override
        protected void paintComponent(Graphics g) {
    ...
    // Represents an interactive icon (GitHub/LinkedIn) with a scaling hover animation and link opening functionality.
    class MagicIcon extends JLabel {
        ...
        // Starts a timer-based animation to smoothly scale the icon size.
        ...
        // Opens the provided URL link in the desktop's default browser.
        ...
        // Custom painting to draw the image with the current scale factor applied.
        @Override
        protected void paintComponent(Graphics g) {
            if (baseImage == null) {
                super.paintComponent(g);
    ...
    // ========= Utilities =========
    // Attempts to load a BufferedImage from the specified resource path, falling back to a default image if unsuccessful.
    ...
    // Creates a circular version of the source image with the specified size.
    ...
    // Creates a custom-styled JButton with an angled border and hover effects.
    ...
    // Data class to hold developer information.
